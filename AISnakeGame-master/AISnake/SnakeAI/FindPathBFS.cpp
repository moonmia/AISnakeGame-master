...
void FindPathBFS::FindPath(COORDINATE begin_point, COORDINATE end_point)
{
	...
	while (!tempque.empty())
	{
		...
		while (quesize--)
		{
			...
			{
				return; //Arrive the distinition
			}
			//Try the current four blocks, right, left, up and down
			for (int i = 0; i < 4; i++)
			{
				...
				if (next_point.x < 0 || 
					...
					m_chess[next_point.x][next_point.y])  //The road is blocked, can't pass
				{
					continue;
				}
...
void FindPathBFS::GetPath(COORDINATE begin_point, COORDINATE end_point)
{
	...
	m_paths_queue.pop(); //Pop out the useless start point.
}
